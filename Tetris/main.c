...
int main()
{
    ...
    SDL_Surface * image = SDL_LoadBMP("KEYS.bmp");
    if (image == NULL) {
    ...
    while (!quit)
    {
		...
		timer -= now - in_time + (score/2000);
		in_time = now;

        if (!tetromino_active->status){
            int cleared_lines = check_grid(grid);
            score += score_count(cleared_lines);
            lines += cleared_lines;
        ...
        show_controls(renderer, font);

        show_score(renderer, font, score);
        show_lines(renderer, font, lines);
        draw_keys(renderer, image);
        if(over == true){
            fprintf(stderr, "Game over!");
            game_over(renderer, font);
            game_over_score(renderer, font, score);
            game_over_text(renderer, font);
